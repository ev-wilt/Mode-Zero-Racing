...
class Game:
    ...
    def countdown(self):
        for i in range(2,-1,-1):
            for n in range(0,5):
                self.screen.blit(pygame.transform.scale(self.numbers[i], (23*n,28*n)), (self.width//2, self.height//2))
                pygame.display.flip()
                pygame.time.wait(100)
                self.screen.fill((0,0,0))
            self.screen.fill((0,0,0))
        self.screen.blit(pygame.transform.scale(self.GO, (300,300)), (self.width//2, self.height//2))
        pygame.display.flip()
